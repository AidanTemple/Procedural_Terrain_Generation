...
bool Map::Initialize(ID3D11Device* device, HWND hwnd, int screenWidth, int screenHeight, D3DXMATRIX viewMatrix, float terrainWidth, 
	float terrainHeight)
{
	bool result;
	...
	result = m_MiniMapBitmap->Initialize(device, screenWidth, screenHeight, L"../Engine/data/colorm01.jpg", 128,128);
	if(!result)
	{
	...
	result = m_Border->Initialize(device, screenWidth, screenHeight, L"../Engine/data/border01.dds", 132, 132);
	if(!result)
	{
...
bool Map::Render(ID3D11DeviceContext* deviceContext, D3DXMATRIX worldMatrix, D3DXMATRIX orthoMatrix, TextureShader* textureShader)
{
	bool result;
